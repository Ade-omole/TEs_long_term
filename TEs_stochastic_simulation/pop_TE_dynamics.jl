...
population_size = 1000  ######## Total population size (number of individuals)
num_diploid_pairs = 1  ######## Each individual has 1 pair of diploid chromosomes (2 haploid chromosomes total)
generations = 800  ######## Number of generations to simulate
...
function crossover(chromosome1, chromosome2)
    num_crossovers = rand(Poisson(0))  ######## Randomly determine the number of crossovers (0 or more)
    if num_crossovers == 0
...
######## Specify the individual and chromosome index to track (e.g., first individual, first chromosome)
tracked_individual_index = 1
tracked_chromosome_index = 1
...
function generate_next_generation(gen)
    ...
    ######## Initialize counts for X and Y for the specific chromosome in the tracked individual
    tracked_X_count = 0
    tracked_Y_count = 0
    generation_C_count = gen == 1 ? 200 : 0

    ######## Create a DataFrame to store individual chromosome counts for the last generation
    individual_chromosome_counts = DataFrame(Individual_ID = Int[], X_1 = Int[], X_2 = Int[], Y_1 = Int[], Y_2 = Int[])

    for i in 1:population_size
        ######## Randomly select two parents for the offspring
        ...
        if crossover_occurred
            ######## Select random chromosomes from recombined parents
            ...
            ######## Select random chromosomes from original parents
            chrom_from_parent1 = rand(1:2)
            chrom_from_parent2 = rand(1:2)
        ...
        ######## Track counts for the specified individual and chromosome
        if i == tracked_individual_index
            tracked_X_count = X_counts[tracked_chromosome_index]
            tracked_Y_count = Y_counts[tracked_chromosome_index]
        end
    ...
    ######## Create a DataFrame for generation-level counts for the tracked chromosome
    generation_data = DataFrame(generation = [gen], X = [tracked_X_count], Y = [tracked_Y_count], Total_C = [generation_C_count])

    ######## Write generation-level counts to a CSV file
    csv_file_path = "/paths/pop_TE_dynamics.csv"
    ...
    ######## Print the tracked chromosome data for the current generation
    println("Generation $gen: Tracked Chromosome X = $tracked_X_count, Tracked Chromosome Y = $tracked_Y_count, Total C = $generation_C_count")
end
